...
statsRouter.get('/week', async (req, res) => {
  try {
    ...
    const total = (pounds + ounces / 16).toFixed(2);
    res.json(total);
  } catch (err) {
    res.status(400).json(err);
  }
...
statsRouter.get('/year', async (req, res) => {
  try {
    ...
    const rate = (((totalTwo - total) / total) * 100).toFixed(2);
    res.json(rate);
  } catch (err) {
    res.status(400).json(err);
  }
...
statsRouter.get('/month', async (req, res) => {
  try {
    ...
    const responseTwo = await pool.query(
      `SELECT SUM(pounds) AS pound_sum, SUM(ounces) AS ounces_sum FROM event_data INNER JOIN events on event_data.event_id=events.id WHERE events.date BETWEEN CURRENT_DATE - INTERVAL '3 months' AND CURRENT_DATE - INTERVAL '1 month' `,
    );
    const poundsTwo = parseFloat(responseTwo.rows[0].pound_sum);
    const ouncesTwo = parseFloat(responseTwo.rows[0].ounces_sum);
    const totalTwo = poundsTwo + ouncesTwo / 16;
